...
func (c *ControllerSystem) ShouldTrack(object entities.GameObject) bool {
	return object.HasComponent(components.PlayerControlName) && object.HasComponent(components.PositionName)
}

func (c *ControllerSystem) Update(dT time.Duration, worldMap cartography.Map, objects map[uint64]entities.GameObject) error {
